          ...
          <div className="container mx-auto px-4 sm:px-6 relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <nav className="flex items-center gap-2 mb-8 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-white/40 overflow-x-auto no-scrollbar whitespace-nowrap">
                ...
                <ChevronRight className="w-3 h-3 text-white/20 shrink-0" />
                <span className="text-aaa-primary uppercase tracking-widest font-black shrink-0">IT Governance</span>
              </nav>
              ...
              <div className="relative">
                <div className="max-w-4xl relative z-10">
                  <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black text-white leading-tight tracking-tight uppercase italic flex flex-col gap-1 md:gap-2">
                    ...
                    <span className="text-2xl sm:text-3xl lg:text-4xl text-aaa-primary opacity-70 font-bold tracking-[0.2em] transition-all">Governance</span>
                  </h1>
            ...
            <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-start">
              ...
              <motion.div className="w-full lg:w-5/12 lg:sticky lg:top-32 h-fit">
                <div className="relative group rounded-[2rem] overflow-hidden shadow-2xl shadow-slate-200">
                  ...
                  <div className="absolute -bottom-6 -right-6 bg-white p-4 sm:p-6 shadow-2xl rounded-xl border border-slate-50 hidden md:block text-aaa-secondary">
                ...
                <div className="mt-8 sm:mt-12 relative">
                  <div className="p-4 sm:p-6 bg-white border border-slate-100 rounded-[1.5rem] sm:rounded-[2rem] shadow-xl shadow-slate-200/50 relative overflow-hidden text-left group">
                    ...
                    <div className="flex items-center gap-3 mb-4 sm:mb-6">
                      ...
                      <h3 className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.3em] italic">Governance Division</h3>
                    </div>
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="w-full lg:w-7/12">
                ...
                <div className="text-slate-600 sm:text-[17px] leading-relaxed mb-12">
                   <p className="font-medium text-slate-800 border-l-4 border-aaa-primary/10 pl-4 sm:pl-6 py-2 mb-8 sm:mb-10 bg-slate-50/50 pr-4 rounded-r-lg italic shadow-sm leading-relaxed text-sm sm:text-base">
                  ...
                  <div className="mb-12 sm:mb-16">
                    <h3 className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.4em] whitespace-nowrap mb-6 sm:mb-8 italic">Value Optimization Objectives:</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      {valueOptimization.map((p, i) => (
                        <div key={i} className="flex items-start gap-4 p-4 sm:p-5 bg-white border border-slate-100 rounded-2xl hover:bg-[#0f172a] transition-all duration-300 group cursor-default shadow-sm italic">
                          <div className="w-1.5 h-1.5 rounded-full bg-aaa-primary mt-2 group-hover:bg-white transition-all" />
                          <span className="text-[11px] sm:text-[13px] font-bold text-slate-700 uppercase tracking-tight group-hover:text-white transition-colors leading-relaxed">{p}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="mb-12 sm:mb-20 p-6 sm:p-10 md:p-14 bg-gradient-to-br from-[#0f172a] to-[#0a0a0a] rounded-[2rem] sm:rounded-[3.5rem] shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-96 h-96 bg-aaa-primary/10 blur-[130px]" />
                    <div className="relative z-10 text-left">
                      <div className="flex items-center gap-5 mb-8 sm:mb-12">
                        <div className="h-[2px] w-12 sm:w-16 bg-aaa-primary/40 rounded-full" />
                        <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-white italic uppercase tracking-tighter leading-tight font-outfit">
                          Why do you need <br /> <span className="text-aaa-primary">IT Governance?</span>
                        </h3>
                      </div>
                  ...
                  <div className="mb-12 sm:mb-20">
                    <h3 className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.4em] mb-6 sm:mb-8 italic text-center">Five Core Pillars</h3>
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-2 sm:gap-3">
                      {governancePillars.map((pillar, idx) => (
                        <div key={idx} className="flex flex-col items-center justify-center p-4 sm:p-6 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-[#0f172a] transition-all group cursor-default shadow-sm">
                          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white flex items-center justify-center mb-3 sm:mb-4 group-hover:bg-aaa-primary transition-colors">
                            <div className="w-1.5 h-1.5 rounded-full bg-aaa-primary group-hover:bg-white" />
                          </div>
                          <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-slate-800 group-hover:text-white text-center">{pillar}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-8 sm:space-y-12">
                    <div className="flex items-center gap-6">
                      <h3 className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.4em] whitespace-nowrap italic">Audit Standards</h3>
                      <div className="h-px w-full bg-slate-100" />
                    </div>

                    <div className="space-y-4 sm:space-y-6">
                      {frameworks.map((f, i) => (
                        <div key={i} className="p-6 sm:p-8 bg-white border border-slate-100 rounded-[2rem] hover:bg-[#0f172a] transition-all duration-300 group shadow-sm italic">
                          <h5 className="font-bold text-slate-900 mb-3 sm:mb-4 text-base sm:text-lg group-hover:text-aaa-primary transition-colors uppercase tracking-tighter">{f.t}</h5>
                          <p className="text-[13px] sm:text-[15px] font-medium text-slate-500 group-hover:text-white/70 leading-relaxed">{f.d}</p>
                        </div>
                      ))}
                    </div>
                  </div>
