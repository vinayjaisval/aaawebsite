              ...
              <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="w-full lg:w-7/12">
                <div className="space-y-4 mb-12">
                  ...
                  <h2 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight leading-tight uppercase italic underline decoration-aaa-primary/10 decoration-8 underline-offset-4">
                    Security <span className="text-aaa-primary">& Control</span>
                  </h2>
                </div>

                <div className="text-slate-600 text-[17px] leading-relaxed mb-12">
                  <p className="font-medium text-slate-800 border-l-4 border-aaa-primary/20 pl-6 py-2 mb-10 bg-slate-50/50 pr-4 rounded-r-lg italic shadow-sm leading-relaxed">"Auditing is a critical security measure to protecting an enterprise in today’s global digital economy."</p>

                  <h3 className="text-xl font-bold text-slate-900 mt-12 mb-8 uppercase tracking-tighter italic underline decoration-aaa-primary/10 decoration-4 underline-offset-4">An Audit in Three Parts</h3>
                  <div className="space-y-6 mb-16">
                    {auditParts.map((item, idx) => (
                      <div key={idx} className="p-8 bg-slate-50/50 border border-slate-100 rounded-3xl hover:bg-[#0f172a] transition-all group cursor-default shadow-sm hover:shadow-xl">
                        <h4 className="font-bold mb-3 uppercase text-[11px] tracking-widest text-aaa-primary transition-all">{item.t}</h4>
                  ...
                  <div className="space-y-12 mb-16">
                    <div className="flex items-center gap-6">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.4em] whitespace-nowrap italic">Primary Security Issues</h3>
                    ...
                    <motion.div variants={{ hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.05 } } }} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid sm:grid-cols-2 gap-4">
                      {securityIssues.map((type, i) => (
                        <motion.div key={i} variants={{ hidden: { opacity: 0, y: 15 }, show: { opacity: 1, y: 0 } }} className="flex items-center gap-4 p-5 bg-white border border-slate-100 rounded-2xl hover:bg-[#0f172a] transition-all duration-300 group cursor-default shadow-sm hover:shadow-lg">
                          <div className="w-1.5 h-1.5 rounded-full bg-aaa-neutral/30 group-hover:bg-white transition-all" />
                          <span className="text-[13px] font-bold text-slate-700 uppercase tracking-tight group-hover:text-white transition-colors leading-relaxed">{type}</span>
                        </motion.div>
                      ))}
                    </motion.div>
                  </div>
                  ...
                  <div className="space-y-12">
                    <div className="flex items-center gap-6">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.4em] whitespace-nowrap italic">Services Provided</h3>
                      <div className="h-px w-full bg-slate-100" />
                    </div>
